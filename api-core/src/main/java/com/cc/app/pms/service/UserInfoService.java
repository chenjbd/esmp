...
public class UserInfoService {
     ...
     * @throws Exception
     */
    public UserLogin checkLogin(String loginId) throws ServiceException {
        UserLogin userLogin = userLoginMapper.selectByPrimaryKey(loginId);
        if(userLogin == null){
            throw new ServiceException("100001","用户未登录");
        }
        Date now = new Date();
        if(userLogin.getExpireTime().compareTo(now) <= 0){
            throw new ServiceException("100001","登录已失效，请重新登录");
        }
        return userLogin;
    }
