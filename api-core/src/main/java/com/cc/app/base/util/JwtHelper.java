package com.cc.app.base.util;

import com.alibaba.fastjson.JSONObject;
import com.cc.app.base.ex.ServiceException;
import com.cc.app.base.utils.EncryptUtil;
...
public class JwtHelper {
     ...
     */
    public static Map<String, Object>  validateJWT(String jsonWebToken, String key) throws ServiceException {
        Map<String, Object> retMap = null;
        if (StringUtils.isBlank(jsonWebToken)) {
            logger.warn("jwt 为空");
            throw new ServiceException("100001", "token为空");
        }
        String[] arr = jsonWebToken.split("\\.");
        if(arr.length != 3){
            logger.error("jwt 无效");
            throw new ServiceException("100001", "token无效");
        }
        //验签
        ...
        if(!sign.equals(arr[2])){
            ...
            throw new ServiceException("100001", "token验签失败");
        }
        //检查是否过期
        ...
        if(expire < System.currentTimeMillis()){
            logger.error("jwt 过期");
            throw new ServiceException("100001", "token已过期");
        }
        return retMap;
    }
